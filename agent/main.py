...
def check_if_exists(get_db_connection, ticker_data):
    ticker_hash = get_ticker_hash(ticker_data)
    if get_db_connection.tickers.find_one({'ticker_hash':ticker_hash}):
        return True

    return False

def save_ticker(get_db_connection, ticker_data=None):
    if not ticker_data:
        return False

    if check_if_exists(get_db_connection, ticker_data):
        return False
    ...
    get_db_connection.tickers.insert_one(ticker_data)
    return True
